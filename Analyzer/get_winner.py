...
def result(match_data, winner):
    result = None
    if match_data['team1_penalty'] is not None and match_data['team2_penalty'] is not None:
        result = f"{match_data['team1']} {match_data['team1_score']}({match_data['team1_penalty']}) - ({match_data['team2_penalty']}){match_data['team2_score']} {match_data['team2']} -> {winner}"
    else:
        result = f"{match_data['team1']} {match_data['team1_score']} - {match_data['team2_score']} {match_data['team2']} -> {winner}"
...
def main():
    ...
    winner = determine_winner(parsed_match_result)
    print(winner)
    result(parsed_match_result, winner)
