...
def parse_ranking_file(file_path='Result.txt'):
    try:
    ...
    for filename in os.listdir(directory):
        if filename.endswith('.json'):
            ...
            try:
                ...
                plt.close()
                print(f'Created professional diagram: {output_path}')
                
            except Exception as e:
                print(f"Failed to create diagram for {filename}: {str(e)}")
                plt.close('all')
...
if __name__ == "__main__":
    ...
    print("Analysis complete! All results saved in:", MAIN_OUTPUT_DIR)